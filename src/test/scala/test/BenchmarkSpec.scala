...
class BenchmarkSpec extends FeatureSpec with GivenWhenThen {
  feature("Benchmark between the input PAR and the output PAR of the two alternatives (with and without prefered slots per user)") {
    scenario("Simple case") {
      ...
      //println(s"IN total PAR = ${Metric.par(SpanSlotAccumulatedLoad(0, users))}")
...
*/
        val resultWithPreferedSlots = Rescheduler.reschedule(user, userPreferedSlots, verbose = true)
      ...
      //println(s"OUT total (without prefered slots) PAR = ${Metric.par(SpanSlotAccumulatedLoad(0, resultsWithoutPreferedSlots))}")
      //println(s"OUT total (with prefered slots) PAR = ${Metric.par(SpanSlotAccumulatedLoad(0, resultsWithPreferedSlots))}")
    }
  }

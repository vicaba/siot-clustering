...
object ClusterRescheduler {
  ...
  def rescheduleCluster(cluster: Cluster, metric: Metric, improvement: Double): (Cluster, List[PointChanged]) = {
    ...
    (Cluster, List[PointChanged]) = {
      ...
      val improvedEnough = improvementPercentage(improvementAchieved) > improvement
      ...
      if (improvedEnough || memory.areAllElementsEqual()) (cluster, changes)
        ...
        val thisImprovement = metric(pointChange.cluster)
        reschedule(thisImprovement, thisImprovement +: memory, pointChange.cluster, pointChanged +: changes)
      }
    }

    reschedule(initialMetric, Memory(3), cluster, List.empty)

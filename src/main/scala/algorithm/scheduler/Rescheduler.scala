...
object Rescheduler {
  ...
  class VectorResult[T](val vector: DenseVector[T],
                        val changedComponent: ChangedComponent,
                        val distanceBeforeReschedule: Double,
                        val distanceAfterReschedule: Double)
  class MatrixResult[T](val matrix: DenseMatrix[T],
                        val row: Int,
                        val changedComponent: ChangedComponent,
                        val distanceBeforeReschedule: Double,
                        val distanceAfterReschedule: Double)
  ...
  /**
    ...
    * @return
    */
  def reschedule(vectorToReschedule: DenseVector[Double],
                 fixedVector: DenseVector[Double],
                 metric: Metric): VectorResult[Double] = {
    ...
    while (i < length) {
      ...
      while (j < length) {
        ...
        if ((distance < smallest) || progression.isPositive) {
          smallest = distance
          bestSolution = vector
  ...
  /**
    ...
    * @return
    */
  def reschedule(matrixToReschedule: DenseMatrix[Double],
                 fixedVector: DenseVector[Double],
                 metric: Metric): Option[MatrixResult[Double]] = {
    ...
    if (bestSolution == null) None
      ...
      Some(
        new MatrixResult[Double](result,
                                 bestSolutionRow,
                                 bestSolution.changedComponent,
                                 bestSolution.distanceBeforeReschedule,
                                 bestSolution.distanceAfterReschedule))
    }
